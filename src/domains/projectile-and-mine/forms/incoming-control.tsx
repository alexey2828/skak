...
const IncomingControl: React.FC<RouteProps> = ({route}) => {
  ...
  const submitForm = async () => {
    try {
      ...
      const requestDataToCommon = {
        ...
        isWater: isWater,
        bodyWeightWithWater: bodyWeightWithWater,
  ...
  useEffect(() => {
    if (bodyWeightWithWater) {
      const weightWaterCalculate =
        Number(bodyWeightWithWater) - Number(weightEemptyBodyWithoutGlassFixed);
      setWeightWater(weightWaterCalculate);
      setChamberV(weightWaterCalculate * 1000);
    }
  }, [bodyWeightWithWater, weightEemptyBodyWithoutGlassFixed]);
